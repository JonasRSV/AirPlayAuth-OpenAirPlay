...
public class AirPlay {
	...
	public AirPlay(String hostname, Socket socket) {
	...
	protected String doHTTP(String method, String uri, ByteArrayOutputStream image, Map<String, String> headers) throws IOException {
		DataOutputStream os = new DataOutputStream(socket.getOutputStream());

		os.writeBytes(method + " "  + uri + " HTTP/1.0\r\n");
		System.out.println(method + " "  + uri + " HTTP/1.0\r\n");
		//os.writeBytes("User-Agent: MediaControl/1.0\r\n");
		os.writeBytes("User-Agent: AirPlay/320.20\r\n");
		...
		os.writeBytes("Content-Length: " + image.size() + "\r\n");
		...
		while ((line = AuthUtils.readLine(this.socket.getInputStream())) != null) {
	...
	protected static Service[] formatSearch(ServiceInfo[] services) throws IOException {
		...
		for (int i = 0; i < services.length; i++) {
			...
			Inet4Address addresses = service.getInet4Address();
			results[i] = new Service(addresses.getHostAddress(), service.getPort(), service.getName());
		}
		return results;
	}
